...
│      Folder       │      Why does this folder need to exist?       │
├───────────────────┼────────────────────────────────────────────────┤
│                   │ The main Django app. All business logic,       │
│ LearningAPI/      │ database models, API endpoints, and tests live │
│                   │  here.                                         │
├───────────────────┼────────────────────────────────────────────────┤
│                   │ The Django project package. Holds settings,    │
│ LearningPlatform/ │ the root URL config, and the WSGI/ASGI entry   │
│                   │ points that boot the server.                   │
├───────────────────┼────────────────────────────────────────────────┤
│                   │ A separate Django app that serves the          │
│ LogViewer/        │ log-browsing web UI. Kept apart from           │
│                   │ LearningAPI/ because it has a different        │
│                   │ responsibility.                                │
├───────────────────┼────────────────────────────────────────────────┤
│                   │ Nginx reverse proxy configs for production.    │
│ config/           │ Needed so the server knows how to route        │
│                   │ traffic to the API and serve the built React   │
│                   │ frontend.                                      │
├───────────────────┼────────────────────────────────────────────────┤
│                   │ Receives runtime log output written by the     │
│ logs/             │ application process. Exists so logs are        │
│                   │ written to a known, persistent location        │
│                   │ outside the source tree.                       │
├───────────────────┼────────────────────────────────────────────────┤
│                   │ Source static asset files before collection.   │
│ static/           │ Django requires a source directory to copy     │
│                   │ from.                                          │
├───────────────────┼────────────────────────────────────────────────┤
│                   │ Output of collectstatic. This is what Nginx    │
│ staticfiles/      │ actually serves in production. Kept separate   │
│                   │ so the web server never needs to touch the     │
│                   │ source directory.                              │
├───────────────────┼────────────────────────────────────────────────┤
│                   │ Django HTML templates for any server-rendered  │
│ templates/        │ pages (e.g. the log viewer UI). Separated from │
│                   │  Python code because Django's template loader  │
...
Folder: models/
What responsibility does it own?: Defines the database schema. Split into
  three sub-packages — people/ (users, cohorts, teams, assessments),
  coursework/ (courses, books, projects, capstones), and skill/
(learning
   records, core skills, weights) — so related tables are grouped by
  domain instead of piled into one file.
─────────────────────
Folder: views/
What responsibility dAPI endpoint handler,
  one file per resource. Also contains sub-folders for GitHub OAuth and
  GitHub integration logic.
────────────────────────────────────────
Folder: serializers/
What responsibility does it own?: Controls how model instances are
  translated to and from JSON. Keeps that conversion logic out of the
  views.
────────────────────────────────────────
Folder: migrations/
What responsibility does it own?: Tracks every schema change to the
  database in order. Django requires this directory to know what has and
  hasn't been applied to the database.
────────────────────────────────────────
Folder: fixtures/
What responsibility does it own?: Holds JSON seed data files, one per
  model, used to populate a fresh database with realistic data for
...
|------|--------------|----------------|
| View |notify.py|notify is a plain function decorated with @api_view(['POST']). It handles exactly one URL and one HTTP method. The entire function is the handler — read the request, do the work, return a response. There is no structure beyond that.
